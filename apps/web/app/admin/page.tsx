...
function Stat({ label, value, sub }: { label: string; value: string; sub?: string }) {
  return (
    <div className="rounded-xl border border-[#E3DED5] bg-white p-5">
      ...
      <div className="mt-1 text-2xl font-semibold text-[#171717]">{value}</div>
      {sub && <div className="mt-1 text-xs text-[#6F6F6F]">{sub}</div>}
    </div>
  );
}
...
export default function Dashboard() {
  ...
  return (
    <div>
      ...
      <p className="text-sm text-[#6F6F6F] mb-6">Genel bakış ve satış özeti.</p>

      {err && <p className="text-[#A23E48] text-sm">{err}</p>}
      {!s && !err && <p className="text-[#6F6F6F] text-sm">Yükleniyor…</p>}
      ...
      {s && (
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mt-4">
            ...
            <div className="rounded-xl border border-[#E3DED5] bg-white p-5">
              <div className="text-xs text-[#6F6F6F]">En popüler etkinlik</div>
              {s.topEvent ? (
                <>
                  <div className="mt-1 text-base font-semibold text-[#171717]">
                    {s.topEvent.title}
                  </div>
                ...
                <div className="mt-1 text-sm text-[#6F6F6F]">—</div>
              )}
            </div>
          </div>
        </>
